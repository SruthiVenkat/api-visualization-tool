...
class d3graph():
    ...
    def show(self, figsize=(1500, 800), title='d3graph', filepath='d3graph.html', showfig=True, overwrite=True, graphLbl=''):
        ...
        self.setup_slider()
        # Create json
        json_data = json_create(self.G, graphLbl)
    ...
    def set_edge_properties(self, hover=None, edge_distance=None, edge_distance_minmax=[1, 20], directed=False, style=None):
        """Edge properties.
        ...
        # Hover text
        if hover:
            edgecount = len(self.edge_properties.keys())
            if isinstance(hover, list):
                hover = np.array(hover).astype(str)
            elif 'numpy' in str(type(hover)):
                pass
            elif isinstance(hover, str):
                hover = np.array([hover] * edgecount)
            else:
                hover = np.array([''] * edgecount)
            if not (len(hover)==edgecount):
                raise Exception("[Hover text] must be of same length as the number of edges")
            for i, edge in enumerate(self.edge_properties):
                self.edge_properties[edge]['hover'] = hover[i]
        else:
            for i, edge in enumerate(self.edge_properties):
                self.edge_properties[edge]['hover'] = ''
        for i, edge in enumerate(self.edge_properties):
            if style:
                self.edge_properties[edge]['style'] = style[i]
            else:
                self.edge_properties[edge]['style'] = 'link'
        

            

    def set_node_properties(self, label=None, hover=None, fixedPos=None, color='#000080', size=10, edge_color='#000000', edge_size=1, cmap='Set1'):
        """Node properties.
...
def json_create(G, graphLbl):
    ...
    links_new = []
    data['graphLbl'] = graphLbl

    for i in range(0, len(links)):
        links[i]['edge_width'] = links[i].pop('weight_scaled')
        links[i]['style'] = links[i].pop('style')
    ...
    for i, node in enumerate(nodes):
        ...
        nodes[i]['fixedPos'] = nodes[i].pop('fixedPos')
...
def edges2G(edge_properties, G=None):
    ...
    for edge in edges:
        G.add_edge(edge[0], edge[1], weight_scaled=np.abs(edge_properties[edge]['weight_scaled']), weight=np.abs(edge_properties[edge]['weight']), color=edge_properties[edge]['color'], hover = edge_properties[edge]['hover'], style = edge_properties[edge]['style'])
    # Return
    return(G)
